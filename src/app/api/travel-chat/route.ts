    ...
    execute: async ({ destination, checkIn, checkOut, budget, preferences, roomType }) => {
      ...
      return {
        ...
        hotels: selectedHotels.map((hotel) => ({
          ...
          description: `${hotel.type} offering ${
            budget === "high"
    ...
    execute: async ({ destination, cuisine, budget, mealType, dietaryRestrictions }) => {
      ...
      return {
        ...
        restaurants: selectedRestaurants.map((restaurant, index) => ({
          ...
          description: `${restaurant.type} serving ${selectedCuisine.toLowerCase()} cuisine with ${
            budget === "high"
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    messages: [
      {
        role: "system",
        content: `You are an expert AI travel planner assistant with deep knowledge of destinations worldwide. You help users plan amazing trips by:
